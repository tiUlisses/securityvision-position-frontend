// src/features/reports/panels/PersonPanel.tsx
...
export default function PersonPanel({ person, from, to, minDurationSeconds }: Props) {
      ...
      try {
        ...
        const [s, hbg, cal, al] = await Promise.all([
          ...
          reportsApi.getPersonHourByGateway(person.id, {
            ...common,
            // para o gráfico de 24h, use sempre uma janela de 24h (independente do filtro geral)
            from_ts: new Date(to.getTime() - 24 * 60 * 60 * 1000).toISOString(),
            to_ts: to.toISOString(),
          }),
          reportsApi.getPersonCalendar(person.id, {
            ...common,
            granularity,
          }),
          reportsApi.getPersonAlerts(person.id, {
            ...common,
            max_events: 500,
          }),
        ]);
  ...
  const hourStack = useMemo(() => {
    ...
    // total por gateway para decidir TOP N
  ...
  const alertsByType = useMemo(() => {
    ...
    list.sort((a, b) => (b.alerts_count ?? 0) - (a.alerts_count ?? 0));
  ...
  return (
    <div className="flex flex-col gap-4">
      <ReportCard
        ...
        right={loading ? <span className="text-[11px] text-slate-400">Carregando...</span> : null}
      >
        ...
        {!person ? (
          ...
          <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <MiniStat label="Primeira sessão" value={summary?.first_session_at ? new Date(summary.first_session_at).toLocaleString() : "-"} />
            <MiniStat label="Última sessão" value={summary?.last_session_at ? new Date(summary.last_session_at).toLocaleString() : "-"} />
          </div>
        )}
      </ReportCard>
            ...
            <div className="h-[300px]">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={dwellByGatewayBar} margin={{ top: 10, right: 10, bottom: 0, left: 0 }}>
                  ...
                  <Bar dataKey="sessions" name="Sessões" />
                </BarChart>
              </ResponsiveContainer>
              <div className="mt-2 text-[11px] text-slate-400">
                Mostrando até 8 gateways. Use a lista abaixo para detalhes.
              </div>
              <div className="mt-2 grid gap-2 sm:grid-cols-2">
                {topGateways.map((g) => (
                  <div key={g.device_id} className="rounded border border-slate-800 bg-slate-900 px-3 py-2 text-[11px] text-slate-300">
            ...
            <div className="h-[300px]">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={hourStack.data} margin={{ top: 10, right: 10, bottom: 0, left: 0 }}>
                  ...
                  <Tooltip />
                  <Legend />
                  {hourStack.keys.map((k) => (
                    <Bar key={k} stackId="a" dataKey={k} name={hourStack.keyToName[k] ?? k} />
                  ))}
                </BarChart>
              </ResponsiveContainer>
              <div className="mt-2 text-[11px] text-slate-400">
                Stack mostra <span className="font-semibold">minutos</span> por hora. Limitado ao TOP 6 gateways.
              </div>
            </div>
          )}
        </ReportCard>
            ...
            <div className="h-[280px]">
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={calendarSeries} margin={{ top: 10, right: 10, bottom: 0, left: 0 }}>
                  ...
                  <Tooltip />
                  <Legend />
                  <Line type="monotone" dataKey="dwell_hours" name="Horas" dot={false} />
                </LineChart>
              </ResponsiveContainer>
            </div>
          )}
